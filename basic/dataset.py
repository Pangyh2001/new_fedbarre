...
def easy_data_partition(n_clients, trainset, testset, shuffle, n_train_data_per_client=1000):
    ...
    # 可调整参数：验证集、测试集大小
    n_val_data_per_client = 500
    n_test_data_per_client = 500
    ...
    logger.info(f"Total train data size: {train_size}, test data size: {test_size}")

    train_indices_all = np.arange(train_size)
    test_indices_all = np.arange(test_size)
    ...
    for i in range(n_clients):
        # 随机采样：训练集来自 trainset，验证/测试集来自 testset
        train_indices = np.random.choice(train_indices_all, size=n_train_data_per_client, replace=True)
        val_indices = np.random.choice(test_indices_all, size=n_val_data_per_client, replace=True)
        test_indices = np.random.choice(test_indices_all, size=n_test_data_per_client, replace=True)

        # 构造数据子集
        train_data = Subset(trainset, train_indices)
        val_data = Subset(testset, val_indices)
        test_data = Subset(testset, test_indices)
        ...
        logger.info(f"Client {i:2d} | Train: {len(train_data)}, Val: {len(val_data)}, Test: {len(test_data)}")
        print_data_distribution(train_data, val_data, test_data)
...
def print_data_distribution(train_data, val_data, test_data):
    ...
    num_classes = max(train_label.keys()) + 1 if train_label else 10
