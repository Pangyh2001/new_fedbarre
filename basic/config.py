...
def extract_nfl_config(args):
    ...
    nfl_cfg.warm_up_rounds = (cfg_dict.get('warm_up_rounds', 8)) #  warm up rounds，这个可能影响实验结果，很大的时候就不会执行barre/nfl/dp了。
    nfl_cfg.u_loss_type = cfg_dict.get('u_loss_type', 'direct')  # direct, gap
    nfl_cfg.privacy = cfg_dict.get('privacy', 'barre')  # TODO nfl, dp, barre,rpf
    nfl_cfg.opt_target = cfg_dict.get('opt_target', 'val')  # val, sigma
    nfl_cfg.clipDP = float(cfg_dict.get('clipDP', 1.0))  # no, nfl, dp-gaussian, dp-laplace
    nfl_cfg.apply_distortion = cfg_dict.get('distort', 'barre')  # no, nfl, dp-gaussian, dp-laplace, barre, rpf
    ...
    nfl_cfg.eps = cfg_dict.get('eps', 0.7)  # epsilon for DP  #TODO 需要修改
    nfl_cfg.D = float(cfg_dict.get('D', 5.6))  # D
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default="cifar10", choices=["fmnist", "mnist", "emnist", "cifar10","cifar100","tiny"])
    ...
    parser.add_argument('--global_epoch', type=int, default=30, help="the epochs for server's training (default: 20)")
    ...
    # args for NFL
    parser.add_argument('--nfl', type=str, required=False, default="nfl", help="nfl custom params")


    # args for RPF
    parser.add_argument('--use_rp', type=str2bool, required=False, default=False, help="Use RPF?") # 这里的False不要带引号，不然就是字符串
    parser.add_argument('--rp_ratio', type=float, default=0.5, help="RPF ratio"),
    parser.add_argument('--noise_type', type=int, default=0, help="noise type for RPF (0=none, 1=normal, 2=op)"),  # rpf是不加噪声的
    parser.add_argument('--rp_eps', type=float, default=1, help="RPF epsilon")    # 这是随机采样权重的方差
    ...
    if 'dp' in args.nfl.apply_distortion:
        ...
        if args.use_rp:
            fname = f'{args.name}_{shuffle_str}_' \
                    f'noisetype{args.noise_type}_' \
                    f'rpratio{args.rp_ratio}_eps{args.nfl.eps}_rpeps{args.rp_eps}_'\
                    f'{int(time.time())}'
        else:
            fname = f'{args.name}_{shuffle_str}_' \
