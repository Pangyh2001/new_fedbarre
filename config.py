...
def extract_nfl_config(args):
    ...
    nfl_cfg.warm_up_rounds = cfg_dict.get('warm_up_rounds', 8) # TODO warm up rounds，这个可能影响实验结果，很大的时候就不会执行barre/nfl/dp了。
    nfl_cfg.u_loss_type = cfg_dict.get('u_loss_type', 'direct')  # direct, gap
    nfl_cfg.privacy = cfg_dict.get('privacy', 'nfl')  # TODO nfl, dp, barre,rpf
    ...
    nfl_cfg.apply_distortion = cfg_dict.get('distort', 'rpf')  #TODO no, nfl, dp-gaussian, dp-laplace, barre, rpf
    ...
    nfl_cfg.eps = cfg_dict.get('eps', 0.9)  # epsilon for DP
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default="mnist", choices=["fmnist", "mnist", "cifar10",])
