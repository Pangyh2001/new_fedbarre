...
class GradientReconstructor():
    ...
    def reconstruct(self, input_data, labels, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None, tb_writer=None, cid=0, bid=0):
        ...
        if self.config['scoring_choice'] in ['pixelmean', 'pixelmedian']:
            ...
            # 修复：处理空的或无效的scores
            valid_scores = scores[torch.isfinite(scores)]  # 过滤掉NaN和Inf
            
            if len(valid_scores) == 0:
                print("Warning: All reconstruction trials failed. Using the last trial result.")
                # 如果所有重建都失败，使用最后一次尝试的结果
                optimal_index = len(scores) - 1
                stats['opt'] = float('inf')  # 标记为失败
                x_optimal = x[optimal_index]
            else:
                # 找到有效分数中的最优结果
                valid_indices = torch.where(torch.isfinite(scores))[0]
                relative_optimal_index = torch.argmin(valid_scores)
                optimal_index = valid_indices[relative_optimal_index]
                
                print(f'Optimal result score: {valid_scores[relative_optimal_index]:2.4f}')
                stats['opt'] = valid_scores[relative_optimal_index].item()
                x_optimal = x[optimal_index]
